...
async def can_zoom_maps(client):
    ...
    frame, expired = client.await_first_element_of(
        [client.css(FRAME_CSS), client.text(EXPIRED_TEXT)], is_displayed=True
    )
    if expired:
        pytest.skip("Site is still apparently expired; cannot run a test now.")
